...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  // Se o usuário forneceu sua própria API key da OpenAI, usá-la diretamente
  const useUserKey = !!userApiKey && userApiKey.trim().length > 0;
  if (!useUserKey) {
    assertApiKey();
  }
  ...
  const apiUrl = useUserKey
    ...
    : resolveApiUrl();
  const authKey = useUserKey ? userApiKey! : ENV.forgeApiKey;
